...
- **Embaixador:** modera só a(s) cidade(s) onde tem uma linha `ambassador_cities` com `status = 'active'`. Acesso restrito a `/embaixador` (seu próprio painel) — **não** ganha acesso a `/admin/*` nem a `/admin/embaixadores`, que continuam exigindo `is_admin`/`is_master` respectivamente. Nenhuma mudança deste spec amplia o acesso de rota de um Embaixador puro.
