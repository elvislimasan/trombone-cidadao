...
test('vencida a escada inteira, não há próxima meta', () => {
  const r = etapaAtual([3, 5, 10], 12);
...
test('missão completa tem barra cheia e rótulo próprio', () => {
  const m = avaliarMissoes({ reports_count: 999 }, 4).find((x) => x.id === 'registrar_broncas');
  assert.equal(m.completa, true);
  assert.equal(m.progresso, 1);
  assert.match(m.rotulo, /concluído/);
});
...
test('dentro da trilha: o que dá para fazer vem antes de troféu e cadeado', () => {
  const c = { patrols_count: 999, total_confirmed: 1 };
  const patrulha = missoesPorTrilha(c, 1).find((t) => t.id === 'patrulha');
  const ordem = patrulha.missoes.map((m) => m.id);

  // 'patrulhar' está completa, 'confirmar_patrulhando' e 'quilometros' pedem
  // nível maior — então nenhuma disponível, mas a completa vem antes das
  // bloqueadas.
  assert.equal(ordem[0], 'patrulhar');
  assert.ok(patrulha.missoes[0].completa);
...
test('missão concluída não promete XP que não existe mais', () => {
  const m = avaliarMissoes({ reports_count: 9999 }).find(
    (x) => x.id === 'registrar_broncas'
  );
  assert.ok(m.completa);
  assert.equal(m.xpAteAEtapa, 0);
});
