...
export default function MunicipalityTeamPage() {
  ...
  const channelMembers = useMemo(
    () => members.filter((member) => String(member.canal_id) === String(channelId)),
    [members, channelId]
  );
  const pendingInvites = useMemo(
    () => invites.filter((invite) => String(invite.canal_id) === String(channelId)),
    [invites, channelId]
  );
    ...
    <>
      ...
      <div className="page-shell-fluid py-6 sm:py-8">
        <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          ...
          {channels.length > 0 && (
            <div className="flex w-full flex-wrap items-end gap-3 xl:w-auto"><div className="min-w-0 flex-1 xl:w-80">
              <Label htmlFor="team-channel">Secretaria</Label>
              <select id="team-channel" value={channelId} onChange={(event) => { setChannelId(event.target.value); setInviteLink(''); }} className="mt-1 h-10 w-full rounded-md border border-edge-strong bg-surface-raised px-3 text-sm">
                {channels.map((channel) => <option key={channel.id} value={channel.id}>{channel.nome}</option>)}
              </select>
            </div><Button onClick={() => setInviteOpen(true)}><Mail className="mr-2 h-4 w-4" />Convidar funcionário</Button></div>
          )}
        </header>
        ...
        {channels.length === 0 ? (
          ...
          <div className="mt-6 min-w-0 space-y-4">
            ...
            <section className="self-start rounded-2xl border border-edge-subtle bg-surface-raised p-5 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-content-tertiary">O convite dá acesso à prefeitura e vincula a pessoa a <strong>{selectedChannel?.nome}</strong>.</p>
                </div>
              </div>
              ...
              <form onSubmit={createInvite} className="mt-5 grid gap-4">
                ...
                <Button type="submit" disabled={working || !email.trim()}>{working && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Gerar convite</Button>
              </form>
            ...
            <MunicipalTable title="Funcionários da secretaria" rows={channelMembers.filter((member) => (statusFilter === 'all' || member.ativo === (statusFilter === 'active')) && (roleFilter === 'all' || member.papel === roleFilter))}
              filterKey={channelId + statusFilter + roleFilter} onOpen={openMember}
              searchPlaceholder="Nome, e-mail ou telefone"
              searchText={(member) => [member.perfil?.name, member.email, member.perfil?.phone].join(' ')}
              filters={<>
              ...
              columns={[
                { key: 'nome', label: 'Nome', width: '23%', value: (member) => member.perfil?.name || 'Servidor municipal', render: (member) => <span className="font-bold text-content-primary">{member.perfil?.name || 'Servidor municipal'}</span> },
                { key: 'email', label: 'E-mail', width: '26%', value: (member) => member.email, render: (member) => <span className="break-all">{member.email || 'Não informado'}</span> },
            ...
            <MunicipalDrawer open={Boolean(selected)} onClose={() => setSelected(null)} busy={saving} title={selected?.perfil?.name || 'Funcionário'} description={selectedChannel?.nome || 'Informações e acesso'}
