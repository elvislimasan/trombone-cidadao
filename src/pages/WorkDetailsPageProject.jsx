...
export default function WorkDetailsPageProject() {
  ...
  const [showCurrentPhaseEditDialog, setShowCurrentPhaseEditDialog] =
    useState(false);
  ...
  const allPayments = useMemo(() => {
    return (measurements || []).flatMap((m) =>
      (m.payments || []).map((p) => ({ ...p, measurement: m }))
    );
  }, [measurements]);
  ...
  const totalExpectedAllPhases = useMemo(() => {
    return (measurements || []).reduce(
      (sum, m) => sum + (Number(m.expected_value) || 0),
      0
    );
  }, [measurements]);
  ...
  const currentPhasePayments = useMemo(() => {
    ...
    return allPayments.filter(
      (p) => p.measurement_id === currentMeasurement.id
    );
  }, [allPayments, currentMeasurement?.id]);

  const currentPhaseTotalPaid = useMemo(() => {
    return currentPhasePayments.reduce(
  ...
  const overallProgress = useMemo(() => {
    const pct =
      currentMeasurement?.execution_percentage ??
      work?.execution_percentage ??
      0;
  ...
  const phase = useMemo(() => {
    ...
    return {
      ...
      contractor: {
        ...
        cnpj: currentMeasurement.contractor?.cnpj
          ? formatCnpj(currentMeasurement.contractor.cnpj)
          : "",
      },
      contractNumber: currentMeasurement.contract_number || "",
      biddingProcessNumber: currentMeasurement.bidding_process_number || "",
      contractPortalLink:
        currentMeasurement.contract_portal_link ||
        currentMeasurement.portal_link ||
        "",
      biddingProcessPortalLink:
        currentMeasurement.bidding_process_portal_link ||
        currentMeasurement.portal_link ||
        "",
  ...
  const phases = useMemo(() => {
    return (measurements || []).map((m) => ({
      ...
      contractor: {
        ...
        cnpj: m.contractor?.cnpj ? formatCnpj(m.contractor.cnpj) : "",
      },
      ...
      biddingProcessPortalLink:
        m.bidding_process_portal_link || m.portal_link || "",
  ...
  const currentGalleries = useMemo(() => {
    ...
    const viewable = (media || []).filter(
      (m) =>
        ["image", "photo", "video", "video_url"].includes(m.type) &&
  ...
  const paymentsForComponent = useMemo(() => {
    const targetMeasurementId =
      paymentTabMeasurementId || currentMeasurement?.id;
    if (!targetMeasurementId) return [];

    const filteredPayments = allPayments.filter(
      (p) => p.measurement_id === targetMeasurementId
    );
    const targetMeasurement = measurements.find(
      (m) => m.id === targetMeasurementId
    );
    ...
    try {
      ...
      const { data: measurementsData, error: measurementsError } =
        await supabase
          ...
          .select(
            "*, contractor:contractor_id(id, name, cnpj), payments:public_work_payments(*)"
          )
          .eq("work_id", workId)
  ...
  const handleUploadGalleryFiles = useCallback(
    async (galleryName, files) => {
      ...
      const targetGalleryName =
        String(galleryName || "").trim() || currentMeasurement.title || "Geral";
      ...
      try {
        for (const file of images) {
          ...
          if (inserted) {
            setMedia((prev) =>
              Array.isArray(prev) ? [inserted, ...prev] : prev
            );
          }
        }
  ...
  const loadWorkOptions = useCallback(async () => {
    const [categoriesRes, areasRes, bairrosRes, contractorsRes] =
      await Promise.all([
    ...
    async (measurementId) => {
      ...
      try {
        ...
        setCurrentPhaseForm({
          ...
          expected_value:
            m.expected_value != null ? formatPtBrMoney(m.expected_value) : "",
          execution_period_days: m.execution_period_days ?? "",
          funding_source: Array.isArray(m.funding_source)
            ? m.funding_source.map((src) =>
                src === "state" ? "estadual" : src
              )
            : [],
          funding_amount_federal:
            m.funding_amount_federal != null
              ? formatPtBrMoney(m.funding_amount_federal)
              : "",
          funding_amount_state:
            m.funding_amount_state != null
              ? formatPtBrMoney(m.funding_amount_state)
              : "",
          funding_amount_municipal:
            m.funding_amount_municipal != null
              ? formatPtBrMoney(m.funding_amount_municipal)
              : "",
  ...
  const handleSaveCurrentPhase = useCallback(async () => {
    ...
    try {
      const payload = {
        title,
        description: currentPhaseForm.description
          ? String(currentPhaseForm.description)
          : null,
        ...
        bidding_process_portal_link:
          currentPhaseForm.bidding_process_portal_link || null,
        portal_link:
          currentPhaseForm.contract_portal_link ||
          currentPhaseForm.bidding_process_portal_link ||
          null,
        ...
        execution_period_days: toIntOrNull(
          currentPhaseForm.execution_period_days
        ),
        funding_source: Array.isArray(currentPhaseForm.funding_source)
          ? currentPhaseForm.funding_source.map((src) =>
              src === "state" ? "estadual" : src
            )
          : [],
        funding_amount_federal: toNumberOrNull(
          currentPhaseForm.funding_amount_federal
        ),
        funding_amount_state: toNumberOrNull(
          currentPhaseForm.funding_amount_state
        ),
        funding_amount_municipal: toNumberOrNull(
          currentPhaseForm.funding_amount_municipal
        ),
        contract_signature_date:
          currentPhaseForm.contract_signature_date || null,
  ...
  const handleAdminSaveWork = useCallback(
    async (workToSave) => {
      try {
        ...
        // Convert empty strings to null for UUID fields to avoid "invalid input syntax for type uuid" error
        [
          "bairro_id",
          "work_category_id",
          "work_area_id",
        ...
        if (cleanedPayload.id === "" || cleanedPayload.id === null) {
    ...
    (payment) => {
      ...
      setPaymentForm({
        ...
        creditor_name:
          ...
          "",
        payment_description:
          raw?.payment_description || payment?.description || "",
  ...
  useEffect(() => {
    ...
    const candidates = allPayments.filter(
      (p) =>
        String(p.commitment_number || "").trim() === commitmentNumber &&
        (p.commitment_date || p.commitment_type)
    );

    const filteredByType = commitmentType
      ? candidates.filter(
          (p) => String(p.commitment_type || "").trim() === commitmentType
        )
      : candidates;

    const typeSet = new Set(
      candidates
        .map((p) => String(p.commitment_type || "").trim())
        .filter(Boolean)
    );
    const dateSet = new Set(
      filteredByType
        .map((p) => String(p.commitment_date || "").trim())
        .filter(Boolean)
    );
    ...
    if (existingPayment) {
      setPaymentForm((prev) => ({
        ...prev,
        commitment_date: !isEditingCommitmentDate
          ? existingPayment.commitment_date || prev.commitment_date
          : prev.commitment_date,
        commitment_type: !isEditingCommitmentType
          ? existingPayment.commitment_type || prev.commitment_type
          : prev.commitment_type,
      }));
    ...
    paymentForm.commitment_number,
    paymentForm.commitment_type,
    allPayments,
    editingPaymentId,
    isEditingCommitmentDate,
    isEditingCommitmentType,
  ]);
    ...
    try {
      ...
      if (editingPaymentId) {
        ...
        if (payload.commitment_date)
          updateObj.commitment_date = payload.commitment_date;
        if (payload.commitment_type)
          updateObj.commitment_type = payload.commitment_type;
        ...
        if (
          ...
          if (batchError)
            console.error("Erro ao atualizar empenho em lote:", batchError);
        }
      } else {
        const { error } = await supabase
          .from("public_work_payments")
  ...
  const openMeasurementDetails = useCallback(
    (measurementId) => {
      const m = measurements.find((x) => x.id === measurementId) || null;
      if (!m) return;
      const phaseMedia = (media || []).filter(
        (x) =>
          x.measurement_id === m.id &&
          ["image", "photo", "video", "video_url"].includes(x.type)
      );
      const phaseDocs = (media || []).filter(
        (x) =>
          x.measurement_id === m.id &&
          (x.type === "pdf" || x.type === "document" || x.type === "file")
      );
  ...
  const timelineItems = [
    {
      ...
      value: formatDateDisplay(phase?.contractSignatureDate),
    },
    {
      label: "Ordem de Serviço",
      value: formatDateDisplay(phase?.serviceOrderDate),
    },
    {
      label: "Previsão Início",
      value: formatDateDisplay(phase?.predictedStartDate),
    },
    {
      label: "Término Prev.",
      value: formatDateDisplay(phase?.expectedEndDate),
    },
    {
      label: "Início Real",
      value: formatDateDisplay(currentMeasurement?.start_date),
    },
    {
      label: "Término Real",
      value: formatDateDisplay(currentMeasurement?.end_date),
    },
    {
      label: "Paralisação",
      value: formatDateDisplay(currentMeasurement?.stalled_date),
    },
    {
      label: "Inauguração",
      value: formatDateDisplay(currentMeasurement?.inauguration_date),
    },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-12 2xl:col-span-9 3xl:col-span-9  space-y-6">
            <section className="bg-card rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge
                      ...
                      className={`${
                        getStatusInfo(
                          normalizeStatus(
                            currentMeasurement?.status || work.status
                          )
                        ).bg
                      } ${
                        getStatusInfo(
              ...
              <div
                ref={currentPhaseTopRef}
                className="border-t pt-6 px-6 mb-6 scroll-mt-24"
              >
                ...
                <p className="text-muted-foreground text-sm xl:text-base leading-relaxed">
                  Confira responsáveis, prazos e cronograma, pagamentos e todas
              ...
              <div className="mx-0 lg:mx-6 border-r-2 shadow-md rounded-xl bg-[#f9fafb] mb-0 lg:mb-8 overflow-hidden min-h-[400px]">
                {currentPhaseView === "general" ? (
                  <>
                    ...
                    <div className="bg-background">
                      <ObraTimeline
                        executionDays={phase?.executionDays || 0}
                        items={timelineItems}
                        embedded
                      />
                    </div>
                    ...
                    <div className="bg-background">
                      <ObraGallery
                        ...
                        emptyMessage={`Nenhuma mídia registrada para ${
                          currentMeasurement?.title || "esta fase"
                        }`}
                    ...
                    <div className="bg-background px-4 sm:px-6 py-6 sm:py-8">
                      ...
                      <Dialog
                        ...
                        onOpenChange={(open) =>
                          !open && setPendingDeleteDocument(null)
                        }
                      >
                  ...
                  <div className="bg-background min-h-[600px] flex flex-col">
                    ...
                    <div className="flex-1">
                      <ObraPayments
                        ...
                        phaseName={
                          measurements.find(
                            (m) => m.id === paymentTabMeasurementId
                          )?.title || ""
                        }
                        embedded
                        measurementId={
          ...
          <div className="order-last lg:order-none lg:col-span-12 2xl:col-span-9">
            ...
            <div className="lg:col-span-12 2xl:col-span-9  3xl:col-span-9 text-center max-w-2xl mx-auto pb-6">
              <p className="text-xs text-muted-foreground leading-relaxed mt-4">
                Os dados são provenientes de portais de transparência e
                verificados pela equipe. Podem haver divergências temporais.
                <button
                  type="button"
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const nextMeasurement =
                      measurements.find((m) => m.id === nextMeasurementId) ||
                      null;
                    setPaymentForm((prev) => ({
                      ...prev,
                      measurement_id: nextMeasurementId,
                      creditor_name: prev.creditor_name
                        ? prev.creditor_name
                        : nextMeasurement?.contractor?.name || "",
                    }));
                  }}
                >
                  {measurements.map((m) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setPaymentForm((prev) => ({
                      ...prev,
                      commitment_number: e.target.value,
                    }))
                  }
                />
              </div>
              ...
              >
                ...
                <Input
                  ...
                  disabled={
                    Boolean(existingCommitmentData.date) &&
                    !isEditingCommitmentDate &&
              ...
              >
                ...
                <select
                  ...
                  disabled={
                    Boolean(existingCommitmentData.type) &&
                    !isEditingCommitmentType &&
                    Boolean(editingPaymentId)
                  }
                  onChange={(e) =>
                    setPaymentForm((prev) => ({
                      ...prev,
                      commitment_type: e.target.value,
                    }))
                  }
                >
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    setPaymentForm((prev) => ({
                      ...prev,
                      value: maskMoneyWhileTyping(e.target.value),
                    }));
                  }}
                  onBlur={() => {
          ...
          <DialogHeader>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              <Badge
                variant="outline"
                className={`${
                  getStatusInfo(selectedMeasurement?.status).color
                } ${getStatusInfo(selectedMeasurement?.status).bg} border-none`}
              >
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="space-y-4">
                        ...
                        selectedMeasurement.funding_source.length > 0 ? (
                          <div className="md:col-span-2 space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {selectedMeasurement.funding_source.map(
                                (rawSrc, idx) => {
                                  const src =
                                    rawSrc === "state" ? "estadual" : rawSrc;
                                  const label =
                                    src === "federal"
                                      ? "Federal"
                                      : src === "estadual"
                                  ...
                                  const amount =
                                    src === "federal"
                                      ? Number(
                                          selectedMeasurement.funding_amount_federal
                                        ) || 0
                                      : src === "estadual"
                                      ? Number(
                                          selectedMeasurement.funding_amount_state
                                        ) || 0
                                      : src === "municipal"
                                      ? Number(
                                          selectedMeasurement.funding_amount_municipal
                                        ) || 0
                                      : 0;
                                  return (
                                    <span
                                      key={`${rawSrc}-${idx}`}
                                      className={`text-xs font-semibold px-2 py-1 rounded-full border ${styles}`}
                                    >
                                      {label}
                                      {amount > 0
                        ...
                        {selectedMeasurement?.stalled_date ? (
                          <div className="col-span-2 sm:col-span-1">
                            ...
                            <span className="font-bold text-red-700 text-sm block">
                              {formatDate(selectedMeasurement.stalled_date)}
                            </span>
                          </div>
                        ) : null}
                      </div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  ...
                  <div className="p-5">
                    {Array.isArray(selectedMeasurement?.payments) &&
                            ...
                            <tbody className="divide-y">
                              {selectedMeasurement.payments.map((p) => (
                                <tr key={p.id} className="text-slate-700">
                                  ...
                                  <td className="p-3 whitespace-nowrap">
                                    {p.commitment_date
                                      ? formatDateDisplay(p.commitment_date)
                                      : "-"}
                                  </td>
              ...
              <div>
                ...
                {Array.isArray(selectedMeasurement?.media) &&
                selectedMeasurement.media.length > 0 ? (
                        ...
                        <div key={group.name} className="space-y-3">
                          ...
                          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                            {group.items.map((mediaItem) => {
                              const originalIndex =
                                selectedMeasurement.media.findIndex(
                                  (m) => m.id === mediaItem.id
                                );
                              return (
                                <div
              ...
              <div>
                ...
                {Array.isArray(selectedMeasurement?.docs) &&
                selectedMeasurement.docs.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <DialogContent className="sm:max-w-md bg-white">
          <DialogHeader>
            ...
            <p className="text-sm text-neutral-500">
              Ajude-nos a manter os dados corretos. Se você identificou algum
              erro nesta obra, por favor nos avise.
            </p>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div className="bg-amber-50 p-4 rounded-lg border border-amber-100 text-amber-800 text-sm">
              <p>
                Ao clicar no botão abaixo, seu gerenciador de e-mail será aberto
                com as informações da obra já preenchidas.
              </p>
            </div>
          </div>
          <DialogFooter>
            ...
            >
              <a
                href={`mailto:contato@trombonecidadao.com.br?subject=Erro na Obra: ${encodeURIComponent(
                  work.title
                )}&body=Olá, gostaria de informar um erro na obra "${
                  work.title
                }" (ID: ${work.id}).%0D%0A%0D%0ADetalhes do erro:%0D%0A`}
              >
                Enviar E-mail
              </a>
            </Button>
        ...
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="grid gap-2 sm:col-span-2">
              <Label>Título</Label>
              <Input
                value={currentPhaseForm.title}
                onChange={(e) =>
                  setCurrentPhaseForm((p) => ({ ...p, title: e.target.value }))
                }
              />
            </div>
            ...
            <div className="grid gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setCurrentPhaseForm((p) => ({ ...p, status: e.target.value }))
                }
              >
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCurrentPhaseForm((p) => ({
                    ...p,
                    bidding_process_number: e.target.value,
                  }))
                }
              />
            </div>
            ...
            <div className="grid gap-2 sm:col-span-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCurrentPhaseForm((p) => ({
                    ...p,
                    contract_portal_link: e.target.value,
                  }))
                }
              />
            </div>
            ...
            <div className="grid gap-2 sm:col-span-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCurrentPhaseForm((p) => ({
                    ...p,
                    bidding_process_portal_link: e.target.value,
                  }))
                }
              />
            </div>
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCurrentPhaseForm((p) => ({
                    ...p,
                    execution_percentage: e.target.value,
                  }))
                }
              />
            </div>
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCurrentPhaseForm((p) => ({
                    ...p,
                    execution_period_days: e.target.value,
                  }))
                }
              />
            </div>
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onBlur={() => {
                  ...
                  setCurrentPhaseForm((p) => ({
                    ...p,
                    expected_value: formatPtBrMoney(n),
                  }));
                }}
              />
            </div>
            ...
            <div className="grid gap-2 sm:col-span-2">
              ...
              <div className="flex flex-wrap gap-4">
                {["federal", "estadual", "municipal"].map((name) => {
                  const checked = (
                    currentPhaseForm.funding_source || []
                  ).includes(name);
                  return (
                    <label
                      key={name}
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={(next) => {
                          setCurrentPhaseForm((p) => {
                            const current = Array.isArray(p.funding_source)
                              ? p.funding_source
                              : [];
              ...
              {Array.isArray(currentPhaseForm.funding_source) &&
              currentPhaseForm.funding_source.length > 0 ? (
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {(currentPhaseForm.funding_source || []).includes(
                    "federal"
                  ) ? (
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setCurrentPhaseForm((p) => ({
                            ...p,
                            funding_amount_federal: next,
                          }));
                        }}
                        onBlur={() => {
                          const n = parsePtBrNumber(
                            currentPhaseForm.funding_amount_federal
                          );
                          if (n == null) return;
                          setCurrentPhaseForm((p) => ({
                            ...p,
                            funding_amount_federal: formatPtBrMoney(n),
                          }));
                        }}
                      />
                    </div>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onBlur={() => {
                          ...
                          setCurrentPhaseForm((p) => ({
                            ...p,
                            funding_amount_state: formatPtBrMoney(n),
                          }));
                        }}
                      />
                    </div>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onBlur={() => {
                          ...
                          setCurrentPhaseForm((p) => ({
                            ...p,
                            funding_amount_municipal: formatPtBrMoney(n),
                          }));
                        }}
                      />
                    </div>
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCurrentPhaseForm((p) => ({
                      ...p,
                      contract_signature_date: e.target.value,
                    }))
                  }
                />
              </div>
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCurrentPhaseForm((p) => ({
                      ...p,
                      service_order_date: e.target.value,
                    }))
                  }
                />
              </div>
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCurrentPhaseForm((p) => ({
                      ...p,
                      predicted_start_date: e.target.value,
                    }))
                  }
                />
              </div>
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCurrentPhaseForm((p) => ({
                      ...p,
                      inauguration_date: e.target.value,
                    }))
                  }
                />
              </div>
            ...
            <div className="grid gap-2">
              ...
              <div className="flex items-center gap-3">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => contribFileInputRef.current?.click()}
                >
                  Selecionar arquivos
                </Button>
                <Input
                ...
                {contribFiles.length > 0 ? (
                  <span className="text-xs text-muted-foreground">
                    {contribFiles.length} arquivo(s) selecionado(s)
                  </span>
                ) : null}
              </div>
            </div>
