      ...
      <CardContent className="p-4 space-y-6">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 border-b pb-2">
            ...
            <h3 className="text-lg font-bold">O que as pessoas estão dizendo</h3>
          </div>
          <ToggleGroup type="single" value={commentSort} onValueChange={(val) => val && setCommentSort(val)} className="justify-start">
            <ToggleGroupItem 
              ...
              className="text-xs font-medium border-b-2 border-transparent data-[state=on]:border-primary data-[state=on]:bg-transparent data-[state=on]:text-primary rounded-none px-2 py-1"
            >
              Recentes
            </ToggleGroupItem>
            <ToggleGroupItem 
        ...
        <div className="grid gap-3">
          {comments.length > 0 ? (
            comments
              ...
              .map((sig, i) => (
              <div key={i} className="flex gap-3 p-4 rounded-xl bg-white border shadow-sm hover:shadow-md transition-all">
                ...
                <div className="space-y-1 w-full">
                  ...
                  {sig.comment && (
                    <p className="text-sm text-foreground/80 leading-relaxed">"{sig.comment}"</p>
                  )}
                </div>
              </div>
            ))
