      ...
      {items.length > 0 ? items.map((report, index) => (
        ...
        >
          <div className="flex items-center gap-2 md:gap-3">
            <div className={`px-2.5 py-1 rounded-full text-[10px] md:text-xs font-bold ${rankBadgeClasses[index] || 'bg-gray-200 text-content-secondary'}`}>
              #{index + 1}
            </div>
    ...
    <Wrapper
      className={
        embedded
          ...
          : 'bg-surface-raised border border-edge-subtle rounded-2xl shadow-sm h-full lg:h-[32rem] xl:h-[32rem] flex flex-col'
      }
    >
      ...
      <CardContent className="flex-1 p-3 pb-0 md:px-6 pt-0 md:pt-0">
        <Tabs defaultValue="upvotes" className="w-full">
          <TabsList className="relative grid w-full grid-cols-2 h-9 md:h-10 bg-transparent p-0 border-b border-border rounded-none">
            <TabsTrigger
              value="upvotes"
              className="gap-1 text-xs md:text-sm rounded-none data-[state=active]:bg-tc-red data-[state=active]:text-white data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-tc-red data-[state=inactive]:text-muted-foreground"
            >
              <ThumbsUp className="w-3 h-3 md:w-4 md:h-4" /> Mais Apoiadas
            </TabsTrigger>
            <TabsTrigger
              value="views"
              className="gap-1 text-xs md:text-sm rounded-none data-[state=active]:bg-surface-raised data-[state=active]:text-tc-red data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-tc-red data-[state=inactive]:text-muted-foreground"
            >
