-- 128_reports_map_clusters_admin_boundaries.sql
--
-- Substitui o agrupamento por grid geográfico fixo (126) por agrupamento
-- em fronteiras administrativas reais, usando reports.city_id / cities.state_id:
--   zoom 0–4  -> 1 cluster por país (hoje só Brasil é suportado)
--   zoom 5–8  -> 1 cluster por estado (state_id)
--   zoom 9–12 -> 1 cluster por cidade (city_id)
--   zoom >=13 -> reports individuais (sem agregação)
--
-- Motivo: em zoom baixo o grid fixo podia cortar uma cidade ao meio em
-- clusters vizinhos; agrupar por fronteira administrativa real é mais
-- intuitivo e casa com a coluna city_id já preenchida em 100% dos reports
-- ativos (backfill da Fase 0 de nacionalização).
...
as $$
  ...
  -- Zoom >= 13: reports individuais completos, sem agregação
  individual as (
    select
      false as is_cluster,
  ...
  -- Zoom 9–12: agrupa por cidade (city_id); reports sem city_id caem num grupo "sem cidade" próprio
  by_city as (
    select
      true as is_cluster,
    ...
    from filtered f
    cross join params
    where params.zoom_sanitized between 9 and 12
    group by coalesce(f.city_id::text, 'no-city-' || f.id::text)
  ),
  -- Zoom 5–8: agrupa por estado (state_id)
  by_state as (
    select
      true as is_cluster,
      avg(lat) as cluster_lat,
    ...
    from filtered f
    cross join params
    where params.zoom_sanitized between 5 and 8
    group by coalesce(f.state_id::text, 'no-state-' || f.id::text)
  ),
  -- Zoom 0–4: um único cluster para todo o país
  by_country as (
    select
      true as is_cluster,
      avg(lat) as cluster_lat,
