-- Corrige o nome exibido de um vereador sem perder a autoria já registrada
-- dentro dos documentos JSON das ruas.
...
declare
  ...
  v_allowed boolean;
begin
  if auth.uid() is null then raise exception 'Faça login para editar o vereador'; end if;
  ...
  if v_councilor.id is null then raise exception 'Vereador não encontrado'; end if;
  ...
  if not v_allowed then raise exception 'Você não tem permissão para editar este vereador'; end if;

  v_new_key := public.normalized_councilor_name(v_new_name);
  if v_new_key is null then raise exception 'Informe um nome válido'; end if;
  if exists (
    ...
    raise exception 'Já existe um vereador com esse nome nesta cidade';
  end if;
