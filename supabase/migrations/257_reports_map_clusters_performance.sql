-- Agrupa ate o zoom 15 e so busca foto/votos para pins realmente individuais.
...
as $$
  ...
  gridded as (
    ...
    cross join lateral (select case p.z
      when 15 then 0.00275 when 14 then 0.0055 when 13 then 0.011
      when 12 then 0.022 when 11 then 0.044 when 10 then 0.088
      else 0.176 end as cell_size) s
    where p.z < 16
  ),
  grouped as (
  ...
  output_rows as (
    ...
    from filtered f cross join params p where p.z >= 16
    union all
